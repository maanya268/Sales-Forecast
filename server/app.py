...
mongoDB=pymongo.MongoClient(os.environ.get("MONGO_URL"))
...
def drop_columns_with_low_uniqueness(df):
    # Calculate the threshold for uniqueness
    threshold = len(df) * 0.05
    
    # Exclude columns containing the substring 'date' from dropping
    # columns_to_drop = [column for column in df.columns if 'date' not in column.lower()]
...
def postPrediction(email):
    ...
    if request.method=="POST":
        ...
        df=drop_columns_with_low_uniqueness(temp_df) 
        for col in df.columns:
            if 'date' in col.lower(): 
